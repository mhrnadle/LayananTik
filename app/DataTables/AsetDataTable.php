...
class asetDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('aset', function ($item) {
                return $item->aset == 1 ? '<span class="badge bg-success">Aktif</span>' : '<span class="badge bg-danger">Non Aktif</span>';
            })
            ->orderColumn('aset', 'aset $1')
            ->rawColumns(['action', 'aset']);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('action')
                ...
                ->width(60)
                ->addClass('text-center'),
            Column::make('NamaAset')
                ->title('Nama Aset')
                ->addClass('text-center'),
            Column::make('JenisAset')
                ->title('Jenis Aset'),
            ...
            Column::make('Kondisi')
                ->title('Kondisi')
                ->addClass('text-center'),
            Column::make('TanggalPembelian')
                ->title('Tanggal Pembelian')
                ->addClass('date-center'),
            Column::make('NilaiAset')
                ->title('Nilai Aset')
                ->addClass('text-center'),
            Column::make('Catatan')
                ->title('Catatan')
